...
for node in whole_tre_unmodified.search_nodes(name="mrcaott3465ott33393"):
    node.name = "Chytridiomycota_ott177160"
    node.props["tx_level"] = "phylum"
    break

for node in whole_tre_unmodified.search_nodes(name="mrcaott7623ott4739017"):
    node.name = "Cryptista_ott1234"
    node.props["tx_level"] = "phylum"
    break

for node in whole_tre_unmodified.search_nodes(name="mrcaott8171ott154619"):
    node.name = "Zygnematophyceae_ott322824"
    node.props["tx_level"] = "class"
    break
...
def get_nodes_for_trimming(parent, to_remove, keep_branches=True):
    ...
    if parent.name == "Streptophyta_ott916750":
        pass
    elif parent.name == "Bacteria_ott844192" or parent.name == "Archaea_ott996421" or parent.name == "Acoelomorpha_ott543718" or parent.name == "Ambulacraria_ott6520512":
        to_remove.append(parent)
        return
    elif (tx_levels[parent.props["tx_level"]] == tx_levels["phylum"] or
          tx_levels[parent.props["tx_level"]] == tx_levels["infrakingdom"]):
        to_remove.append(parent)
        return
...
to_remove = []
get_nodes_for_trimming(whole_tre, to_remove, keep_branches=True)
...
#     tree_fixing.remove_tree_below(node)
    ...
    # if node.name == "Bacteria_ott844192" or node.name == "Archaea_ott996421":
    #     continue

    # DON'T DO THIS and see what is actually left
    # if (tx_levels[node.props["desc_rank"]] < tx_levels["phylum"] or
    #     tx_levels[node.props["desc_rank"]] > tx_levels["infrakingdom"] or
    #     node.props["num_leaves"] < 10):
...
# euk_tre = tree_fixing.delete_one_child_nodes(euk_tre, maintain_branch_lengths=True)
