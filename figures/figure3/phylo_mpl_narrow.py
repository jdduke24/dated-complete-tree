...
def plot_tree(tree, cmap, ax=None, figsize=(10, 8), align_labels=True):
    ...
    for idx, node in enumerate(tree.traverse(strategy="levelorder")):
        ...
        line_width = 1 + pd_conversion*0.5
        # line_width = 2
    ...
    if not align_labels:
        ...
        for leaf in tree.leaves():
            ...
            tri_pd_conv = 0.45*pd_conversion + 0.55

            # tri_x_size = 0.01
            tri_y_size = 0.6

            # x_tri = (x-tri_x_size)+(tri_x_size-tri_x_size*tri_pd_conv/2)
            x_tri = min_x - 0.01
            y_tri = (y-tri_y_size/2)+(tri_y_size-tri_y_size*tri_pd_conv)/2

            path = Path([[x_tri, y_tri],
                         [x_tri, y_tri+tri_y_size*tri_pd_conv],
                         [x+0.005, y_tri+tri_y_size*tri_pd_conv*3/5],
                         [x+0.005, y_tri+tri_y_size*tri_pd_conv*2/5],
                         [x_tri,y_tri]])
        ...
        for node in tree.traverse():
            ...
            circ_x_size = 0.02
            circ_y_size = circ_x_size*50

            circ = patches.Ellipse((x,y),
                                    circ_x_size*pd_conversion,
                                    circ_y_size*pd_conversion,
                                    fill=False,
                                    rasterized=True,
                                    linewidth=0.6,
                                    edgecolor="dimgrey",
    ...
    x_ticklabels = [str(x) for x in yrs]
    x_ticklabels[14] = "1800 "
    x_ticklabels[19] = "3600 "
    ax.set_xticklabels(x_ticklabels, fontfamily="Arial", fontsize="x-small")
...
fig.tight_layout()
fig.savefig("figures/figure3/mpl_tri_nodes5_narrow_calib.tif", dpi=300)
fig.savefig("figures/figure3/mpl_tri_nodes5_narrow_calib.pdf", dpi=300)
